...
ombvec=np.linspace(0.005,0.025,21)
pp=np.asarray(pars)
models=[None]*len(ombvec)
for i,omb in enumerate(ombvec):
    pp[1]=omb
    model=get_spectrum(pars)
    model=model[:len(spec)]
...
while True:
    for i in range(len(ombvec)):
        plt.clf()
        plt.errorbar(planck_binned[:,0],planck_binned[:,1],errs_binned,fmt='.')
        plt.plot(ell,models[i],'k')
        plt.ylim([0,6000])
        plt.text(1500,3000,r'$\Omega_Bh^2=$'+f'{ombvec[i]:.3f}',fontsize=20)
